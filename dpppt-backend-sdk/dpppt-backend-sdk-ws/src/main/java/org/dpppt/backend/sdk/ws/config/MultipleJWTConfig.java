...
public class MultipleJWTConfig {
	...
	private static final Logger log = LoggerFactory.getLogger(MultipleJWTConfig.class);
	...
	public static class CommonJWTBase extends WebSecurityConfigurerAdapter {
		...
		protected String loadPublicKey() throws IOException {
			log.debug("loadPublicKey (ws.app.jwt.publickey): {}", publicKey);
	...
	// @formatter:on
		...
		public ValidateRequest gaenRequestValidator() {
			return new org.dpppt.backend.sdk.ws.security.gaen.JWTValidateRequest(validationClientService);
		}
		...
		public JwtDecoder jwtDecoder() throws InvalidKeySpecException, NoSuchAlgorithmException, IOException,
			...
			String loadPK = loadPublicKey();
			log.debug("Creating jwtDecoder with algorithm {} and publicKey {}", algorithm, loadPK);
			DPPTJwtDecoder jwtDecoder = new DPPTJwtDecoder(KeyVault.loadPublicKey(loadPK, algorithm));
